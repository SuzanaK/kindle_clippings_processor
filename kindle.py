...
def process_file():
    ...
    for ebook, clipping_list in ebooks.items():
        ...
        ebook = "".join([c if c.isalnum() else " " for c in ebook])
        ...
        if not timestamp or not timestamp.year:
            ...
            year = timestamp.year
        markdown_file_path: Path = output_path.joinpath(str(year)).joinpath(ebook + '.md')
        fh = codecs.open(markdown_file_path, 'w', 'utf-8')
        print(str(markdown_file_path))
